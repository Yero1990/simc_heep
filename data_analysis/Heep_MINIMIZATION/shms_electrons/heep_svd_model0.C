...
//****IMPORTANT*** :: ONLY PARAMETERS a0, a1 --> dEf/Ef and dth_e are considered
//Here,  the 12 equations are treated separately, NOT summed over all runs.

void heep_svd_model0()
{
  ...
  TMatrixD C(12,2);     //4 quations / run, for 3 elastic runs --> total 12 equations
  TVectorD bVec(12);
  TVectorD bVec_err(12);
  TVectorD bVec_cpy(12);

  TVectorD aVec(2);  //vector to store the parameters (a0, a1, a2, a3)
  ...
  //Get Covariance Matrix: V(2x2) = CT(2x12) * R1(12x2) 
  for(int i=0; i<2; i++){
  ...
  for(int i=0; i<12; i++)
    {
      ...
      dObs_model = C[i][0] * aVec[0] + C[i][1] * aVec[1] ;
      //Model Error (Include correlated errors)
      dObs_model_err = sqrt( pow(C[i][0] * a0_err, 2) + pow(C[i][1] * a1_err,2)
			     + 2.*C[i][0]*C[i][1]*Vinv[0][1]);
  ...
  cout << "total equations x total runs = 4x3 = 12 observations, # parameters = 2, dof = 10 " << endl;
