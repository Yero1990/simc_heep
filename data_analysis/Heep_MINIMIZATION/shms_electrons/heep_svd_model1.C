...
//****IMPORTANT*** :: ONLY PARAMETERS a0, a1, a2 --> dEb/Eb,  dEf/Ef and dth_e are considered
//Here,  the 12 equations are treated separately, NOT summed over all runs.

void heep_svd_model1()
{
  ...
  //Get Covariance Matrix: V(3x3) = CT(3x12) * R1(12x3) 
  for(int i=0; i<3; i++){
  ...
  for(int i=0; i<12; i++)
    {
      ...
      dObs_model_err = sqrt( pow(C[i][0] * a0_err, 2) + pow(C[i][1] * a1_err,2) + pow(C[i][2] * a2_err, 2) 
			     + 2.*C[i][0]*C[i][1]*Vinv[0][1] + 2.*C[i][0]*C[i][2]*Vinv[0][2] + 2.*C[i][1]*C[i][2]*Vinv[1][2]); 
