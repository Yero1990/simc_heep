...
#fname = 'hms_kin_proton.data'
fname = '../kinfile/elastic_shms.data'

f = B.get_file(fname)
kg = B.get_data(f, 'kg')                  #kin. group
run = B.get_data(f, 'Run')
particle = B.get_data(f, 'particle')
hmsP = 0                   #HMS Momentum [GeV]
hmsAng = 0           #HMS Angle [deg]
...
simcW_err = 0
dataPfr = 0       #Data fractional momentum (Pcacl - Pmeas) / Pmeas mean of fit
dataPfr_err = 0
simcPfr = 0
simcPfr_err = 0
...
step_p1 = 0.00005     #(steps of 0.02%, for example) 
step_p2 = 0.00005     #0.0001
step_p3 = 0.00005     #0.0001
    ...
    for n in frange(p2_min, p2_max+step_p2, step_p2):
        for l in frange(p3_min, p3_max+step_p3, step_p3):
            ...
            for i, ikg in enumerate(kg):
                ...
                th = shmsAng[i] * dtr     #convert SHMS angle to radians

                p1 = m    #relative uncertainty in Beam Energy (%/100)
                p2 = n    #relative uncertainty in sHMS momentum (either proton or electron, %/100)
                p3 = l    #relative uncertainty in sHMS angle (radians)
                ...
                if particle[i]=='e':
                    ...
                    dW_meas = dataW[i] - simcW[i]
                    dW_err = beamE[i] * 8.*1e-4 /shmsP[i] #np.sqrt(simcW_err[i]**2 + dataW_err[i]**2)
                
                    #Derivative dW/dEb, 
                    dW_dEb = shmsP[i]/beamE[i]
                
                    #Derivative dW/dE', 
                    dW_dP = -beamE[i]/shmsP[i]
                
                    #Derivative dW/dth_e
                    dW_dth = -2 * beamE[i] * shmsP[i] * np.sin(th/2) * np.cos(th/2) / Mp
