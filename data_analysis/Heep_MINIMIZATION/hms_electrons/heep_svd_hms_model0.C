...
//MODEL 0:  This model useds derivatives of W in terms of (Ef, th_e) to describe any changes observed in the obsereved dW=data-simc
//Beam Energy is assumed to be well knwon

void heep_svd_hms_model0()
{
  ...
  TMatrixD C(sel,2);     //11 runs, 11 equations, 2 parameters
  TVectorD bVec(sel);     //observed vector
  TVectorD bVec_err(sel);     //observed vector
  
  TVectorD bVec_cpy(sel);  //copy
  
  TVectorD aVec(2);  //vector to store the parameters (a0, a1, a2)
  ...
  Double_t a0, a1;   //a0 --> dEf/Ef,  a1 --> dth_e [rad]
  ...
  ofstream ofs;
  ofs.open("hms_residuals_model0.txt");
  ...
  for(int i=0; i<sel; i++)
    {
      ...
      dW_model = C[i][0] * aVec[0] + C[i][1] * aVec[1];
      dW_model_err = sqrt( pow(C[i][0] * a0_err, 2) + pow(C[i][1] * a1_err,2) 
			   + 2.*C[i][0]*C[i][1]*Vinv[0][1]); //correlated errors
  ...
  cout << "total equations = 11 observations, # parameters = 2, dof = 9 " << endl;
