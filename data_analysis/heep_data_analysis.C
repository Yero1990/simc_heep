...
//  TFile *data_file = new TFile("../../hallc_replay/ROOTfiles/coin_replay_production_1929_-1.root", "READ");
  ...
  TFile *outfile = new TFile("heep_data_1929.root", "recreate");
  ...
  //********* Create 1D Histograms **************
   ...
   TH1F *data_omega = new TH1F("data_omega","Energy Transfer, #omega", bins, 0., 4.);
   ...
   TH1F *data_hytar = new TH1F("data_hytar", "SHMS Y_{tar}", bins, -4., 4.);
   TH1F *data_hxptar = new TH1F("data_hxptar", "SHMS X'_{tar}", bins, -0.08, 0.08 );
   TH1F *data_hyptar = new TH1F("data_hyptar", "SHMS Y'_{tar}", bins, -0.04, 0.04 );
   TH1F *data_hdelta = new TH1F("data_hdelta", "SHMS Momentum Acceptance, #delta", bins, -10., 10. );

   //Hadron arm Focal Plane Quantities
   TH1F *data_hxfp = new TH1F("data_hxfp", "SHMS X_{fp}", bins, -30., 20.);
   TH1F *data_hyfp = new TH1F("data_hyfp", "SHMS Y_{fp}", bins, -15., 10.);
   TH1F *data_hxpfp = new TH1F("data_hxpfp", "SHMS X'_{fp}", bins, -0.06, 0.06 );
   TH1F *data_hypfp = new TH1F("data_hypfp", "SHMS Y'_{fp}", bins, -0.03, 0.02);

      
   //Electron Arm Reconstructed Quantities ( xtar, ytar, xptar, yptar, delta)
   TH1F *data_eytar = new TH1F("data_eytar", "HMS Y_{tar}", bins, -4., 4.);
   TH1F *data_exptar = new TH1F("data_exptar", "HMS X'_{tar}", bins, -0.08, 0.08);
   TH1F *data_eyptar = new TH1F("data_eyptar", "HMS Y'_{tar}", bins, -0.04, 0.04);
   TH1F *data_edelta = new TH1F("data_edelta", "HMS Momentum Acceptance, #delta", bins, -15., 15. );

   //Electron Arm Focal Plane Quantities
   TH1F *data_exfp = new TH1F("data_exfp", "HMS X_{fp}", bins, -60., 30.);
   TH1F *data_eyfp = new TH1F("data_eyfp", "HMS Y_{fp}", bins, -20., 30.);
   TH1F *data_expfp = new TH1F("data_expfp", "HMS X'_{fp}", bins, -0.1, 0.06);
   TH1F *data_eypfp = new TH1F("data_eypfp", "HMS Y'_{fp}", bins, -0.04, 0.04);
   ...
   T->Draw("P.kin.secondary.emiss>>data_Emiss");
   T->Draw("P.kin.secondary.pmiss>>data_pm");
   T->Draw("H.kin.primary.Q2>>data_Q2");
   T->Draw("H.kin.primary.nu>>data_omega");
   T->Draw("H.kin.primary.W>>data_W_inv");
   T->Draw("H.kin.primary.scat_ang_rad>>data_theta_elec");
