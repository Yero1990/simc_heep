...
void analyze_singlesData(int runNUM, int evtNUM)
{
  ...
  //Kinematics Quantities
   ...
   TCut em_cut = "H.dc.1u1.nhit==1&&H.dc.1u2.nhit==1&&H.dc.1v1.nhit==1&&H.dc.1v2.nhit==1&&H.dc.1x1.nhit==1&&H.dc.1x2.nhit==1&&H.dc.2u1.nhit==1&&H.dc.2u2.nhit==1&&H.dc.2v1.nhit==1&&H.dc.2v2.nhit==1&&H.dc.2x1.nhit==1&&H.dc.2x2.nhit==1&&H.kin.x_bj>0.9";
   TCut Q2_cut = Form("%s.kin.Q2>3&&%s.kin.Q2<5.0", primary.c_str(), primary.c_str());
   TCut xbj_cut = Form("%s.kin.x_bj>0.7&&%s.kin.x_bj<1.3", primary.c_str(), primary.c_str()); 
   ...
   T->Draw(Form("%s.kin.Q2>>data_Q2", primary.c_str()), em_cut);
   T->Draw(Form("%s.kin.nu>>data_omega", primary.c_str()), em_cut);
   T->Draw(Form("%s.kin.W>>data_W_inv", primary.c_str()), em_cut);
   T->Draw(Form("(%s.kin.scat_ang_rad*180./3.14)>>data_theta_elec", primary.c_str()), em_cut);

   //T->Draw(Form("(%s.kin.secondary.xangle - %s.kin.scat_ang_rad)*180./3.14>>data_theta_prot", secondary.c_str(), primary.c_str()), em_cut); 
   
   //Additional Kinematic Variables
   T->Draw(Form("%s.kin.W2>>data_W2", primary.c_str()), em_cut);
   T->Draw(Form("%s.kin.x_bj>>data_xbj", primary.c_str()), em_cut);
   //T->Draw(Form("%s.gtr.p>>data_Pf", secondary.c_str()), em_cut);
   T->Draw(Form("%s.gtr.p>>data_kf", primary.c_str()), em_cut);
   T->Draw(Form("%s.kin.th_q*(180./3.14)>>data_theta_q", primary.c_str()), em_cut ); //theta_q
