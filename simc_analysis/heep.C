//Analysis for COIN. runs 1854 AND 1866, shms->electrons, hms->protons
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TH1F *hytar = new TH1F("hytar", "SHMS Y_{tar}", bins, -4., 4.);
   TH1F *hxptar = new TH1F("hxptar", "SHMS X'_{tar}", bins, -0.08, 0.08 );
   TH1F *hyptar = new TH1F("hyptar", "SHMS Y'_{tar}", bins, -0.04, 0.04 );
   TH1F *hdelta = new TH1F("hdelta", "SHMS Momentum Acceptance, #delta", bins, -10., 10. );

   //Hadron arm Focal Plane Quantities
   TH1F *hxfp = new TH1F("hxfp", "SHMS X_{fp}", bins, -30., 20.);
   TH1F *hyfp = new TH1F("hyfp", "SHMS Y_{fp}", bins, -15., 10.);
   TH1F *hxpfp = new TH1F("hxpfp", "SHMS X'_{fp}", bins, -0.06, 0.06 );
   TH1F *hypfp = new TH1F("hypfp", "SHMS Y'_{fp}", bins, -0.03, 0.02);

      
   //Electron Arm Reconstructed Quantities ( xtar, ytar, xptar, yptar, delta)
   TH1F *eytar = new TH1F("eytar", "HMS Y_{tar}", bins, -4., 4.);
   TH1F *exptar = new TH1F("exptar", "HMS X'_{tar}", bins, -0.08, 0.08);
   TH1F *eyptar = new TH1F("eyptar", "HMS Y'_{tar}", bins, -0.04, 0.04);
   TH1F *edelta = new TH1F("edelta", "HMS Momentum Acceptance, #delta", bins, -15., 15. );

   //Electron Arm Focal Plane Quantities
   TH1F *exfp = new TH1F("exfp", "HMS X_{fp}", bins, -60., 30.);
   TH1F *eyfp = new TH1F("eyfp", "HMS Y_{fp}", bins, -20., 30.);
   TH1F *expfp = new TH1F("expfp", "HMS X'_{fp}", bins, -0.1, 0.06);
   TH1F *eypfp = new TH1F("eypfp", "HMS Y'_{fp}", bins, -0.04, 0.04);
