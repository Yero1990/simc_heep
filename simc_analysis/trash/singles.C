...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TH1F *Emiss = new TH1F("Emiss","missing energy", Em_nbins, -0.5, 0.8);  //binwidth = 0.0025
   ...
   TH1F *hytar = new TH1F("hytar", h_arm + " Y_{tar}", bins, -4., 4.);
   TH1F *hxptar = new TH1F("hxptar", h_arm + " X'_{tar}", bins, -0.08, 0.08 );
   TH1F *hyptar = new TH1F("hyptar", h_arm + " Y'_{tar}", bins, -0.04, 0.04 );
   TH1F *hdelta = new TH1F("hdelta", h_arm + " Momentum Acceptance, #delta", bins, -10., 10. );

   //Hadron arm Focal Plane Quantities
   TH1F *hxfp = new TH1F("hxfp", h_arm + " X_{fp}", bins, -30., 20.);
   TH1F *hyfp = new TH1F("hyfp", h_arm + " Y_{fp}", bins, -15., 10.);
   TH1F *hxpfp = new TH1F("hxpfp", h_arm + " X'_{fp}", bins, -0.06, 0.06 );
   TH1F *hypfp = new TH1F("hypfp", h_arm + " Y'_{fp}", bins, -0.03, 0.02);

      
   //Electron Arm Reconstructed Quantities ( xtar, ytar, xptar, yptar, delta)
   TH1F *eytar = new TH1F("eytar", e_arm + " Y_{tar}", bins, -4., 4.);
   TH1F *exptar = new TH1F("exptar", e_arm + " X'_{tar}", bins, -0.08, 0.08);
   TH1F *eyptar = new TH1F("eyptar", e_arm + " Y'_{tar}", bins, -0.04, 0.04);
   TH1F *edelta = new TH1F("edelta", e_arm + " Momentum Acceptance, #delta", bins, -40., 40. );

   //Electron Arm Focal Plane Quantities
   TH1F *exfp = new TH1F("exfp", e_arm + " X_{fp}", bins, -40., 40.);
   TH1F *eyfp = new TH1F("eyfp", e_arm + " Y_{fp}", bins, -40., 40.);
   TH1F *expfp = new TH1F("expfp", e_arm + " X'_{fp}", bins, -0.1, 0.1);
   TH1F *eypfp = new TH1F("eypfp", e_arm + " Y'_{fp}", bins, -0.04, 0.04);
