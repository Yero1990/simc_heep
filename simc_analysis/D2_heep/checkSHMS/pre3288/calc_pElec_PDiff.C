...
void calc_pElec_PDiff()
{
  ...
  Double_t eP0[4] = {8.532738,8.544319,8.544319,8.544319};  //shms central momentum corrected (before SHMS delta optimization)

  Double_t hP0[4] = {2.929192, 3.4679035, 2.3048787, 1.8865303};  //central spec. momentum (Corrected HMS)
  ...
  int run_num[4] = {3248, 3371, 3374, 3377};
  ...
  for(int irun = 0; irun<1; irun++)
    {
      ...
      string filename = Form("../../../../worksim_voli/D2_heep_%d.root", run_num[irun]);        //SHMS Corrected Momentum (Emiss aligned) 
	...
	//(shms_delta_calc - shms_delta_meas) vs. shms focal plane                                                                       
      ...
      if(irun==0){                                                                                                             
        hist_ePDev_xfp[irun] = new TH2F(Form("ePDiff_vs_xfp: Run %d", run_num[irun]), "", 100, -10., 5, 80, -0.005, 0.005);      
        hist_ePDev_xpfp[irun] = new TH2F(Form("ePDiff_vs_xpfp: Run %d", run_num[irun]), "", 100, -0.04, 0.03, 80, -0.005, 0.005);          
        hist_ePDev_yfp[irun] = new TH2F(Form("ePDiff_vs_yfp: Run %d", run_num[irun]), "", 100, -10, 5, 80, -0.005, 0.005);         
        hist_ePDev_ypfp[irun] = new TH2F(Form("ePDiff_vs_ypfp: Run %d", run_num[irun]), "", 100, -0.02, 0.02, 80, -0.005, 0.005);            
	...
	//Fill Histograms
	if (Em<0.04 && simc_hmsDelta_cut && simc_shmsDelta_cut && simc_hmsPDiff_cut){
  ...
  Double_t Em_cut_arr[4] = {0.04, 0.05, 0.05, 0.05};      //Em cuts after SECOND SHMS DELTA OPTIMIZATION
...
//Define some variables to be determined inside the entry loop
  ...
  ofile << "#Run 3248: mean: " << yRes_diff[0] << " err: " << yRes_diff_err[0] << endl;

  ofile << "" << endl;
  ofile << "#Momentum Correction Factor: " << endl;
  ofile << "#del = (simc - data)" << endl;
  ofile << "#corr_factor = 1 - (del/Pcentral)" << endl;
  ofile << "#3248: "<< 1. - (yRes_diff[0]/eP0[0]) << "   +/-  " <<  yRes_diff_err[0]/eP0[0] << "  [GeV]  " <<  endl;
