...
void calc_pElec_PDiff()
{
  ...
  //Double_t eP0[4] = {8.5342488, 8.5342488,8.5342488,8.5342488};  //shms central momentum corrected (before SHMS delta optimization)

  //Double_t hP0[4] = {2.935545, 3.47588, 2.3103874, 1.891227};  //central spec. momentum (Corrected HMS)
  Double_t hP0[4] = {2.938,  3.48, 2.31, 1.8899};  //UnCorrected, EPICS
 ...
 //Define some variables to be determined inside the entry loop
  ...
  for(int irun = 0; irun<4; irun++)
    {
      ...
      string filename = Form("../../../worksim_voli/D2_heep_%d_ORIGINAL.root", run_num[irun]);        //SHMS Corrected Momentum (Emiss aligned) 
  ...
  //Em cut array
  Double_t Em_cut_arr[4] = {-0.125, -0.115, -0.14, -0.14}; //Initial
  ...
  //Double_t Em_cut_arr[4] = {0.04, 0.04, 0.04, 0.04};      //Em cuts after SECOND SHMS DELTA OPTIMIZATION
...
//Define some variables to be determined inside the entry loop
  ...
  ofile << "#3288: "<< 1. - (yRes_diff[0]/eP0[0]) << "   +/-  " <<  yRes_diff_err[0]/eP0[0] << "  [GeV]  " <<  endl;
  ofile << "#3371: "<< 1. - (yRes_diff[1]/eP0[1]) << "   +/-  " <<  yRes_diff_err[1]/eP0[1] << "  [GeV]  " <<  endl;
  ofile << "#3374: "<< 1. - (yRes_diff[2]/eP0[2]) << "   +/-  " <<  yRes_diff_err[2]/eP0[2] << "  [GeV]  " <<  endl;
  ofile << "#3377: "<< 1. - (yRes_diff[3]/eP0[3]) << "   +/-  " <<  yRes_diff_err[3]/eP0[3] << "  [GeV]  " <<  endl;
